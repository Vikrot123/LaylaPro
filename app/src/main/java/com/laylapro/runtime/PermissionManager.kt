...
class PermissionManager(private val context: Context) {
    ...
    fun isGranted(permission: LaylaPermission): Boolean = when (permission) {
        ...
        // MediaProjection is an ephemeral, user-approved session token, not a durable
        // manifest permission. This manager cannot truthfully report it as granted.
        LaylaPermission.MEDIA_PROJECTION -> false
    }

    fun requireGranted(permission: LaylaPermission, requestedBy: String): Boolean {
        val granted = isGranted(permission)
        if (!granted) EventBus.tryPublish(CoreEvent.PermissionDenied(permission.name, requestedBy))
        return granted
    }
