 ...
 * ни `agent`. До этого класса код пакета `agent` напрямую импортировал
 ...
 * а код пакета `agent` зависит только от этого узкого контракта, а не от конкретного
 * класса в `runtime`. Это разрывает цикл: `agent -> logging`, `runtime -> logging`,
